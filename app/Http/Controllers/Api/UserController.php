...
class UserController extends BaseController
{
    ...
    private function loopUsersActive( string $codeUuid , $points, $countUserActive = 0): int
    {
        ...
        {
            $_userModel = User::with(['paymentActive'])->where('uuid', 'like', $userSponsor->user_code)->first();
    ...
    public function modifyUser( Request $request )
    {
        ...
        try {
            ...
            if (!empty(trim($dataBody->userDni))) {
                if( $dataBody->userDni != $userUpdated->uuid){
    ...
    public function changeSponsor(Request $request)
    {
        ...
        try {
            ...
            $paymentOrder = PaymentOrder::where("id" , $paymentLog->paymentOrder->id)->first();
        ...
        try {
            ...
            if( $personalPoint >= floatval($maxPointsProduct->option_value) )
            {
                ...
                if( $__paymentLog != null ){
                    ...
                    $this->paymentOrderService->confirmPoint($_paymentOrder , $userUpdated , $paymentLog->paymentOrder->pack, true);
    ...
    public function exportExcelFinance(Request $request)
    {
        try {
            ...
            Excel::store(new ReportExcelUsers($excelBody), $nameFile, 'local');

            $path = Storage::disk('local')->path($nameFile);
            $fileContents = file_get_contents($path);

            $base64 = base64_encode($fileContents);

            Storage::disk('local')->delete($nameFile);
    ...
    public function exportPdfProfile(Request $request)
    {
        try {
            ...
            $data = array(
                ...
                "residualTotal" => $_pointTemp['points']?->residualTotal ?? 0,
                "currentPack"    => $_pointTemp['points']?->currentPack ?? 0,
                "residualVolumen" => $_pointTemp['points']?->residualVolumen ?? 0,
    ...
    public function createUser(Request $request)
    {
        try {
            ...
            if( $dataBody->plan != null ){
                ...
                $this->paymentOrderService->totalProductPatrocinio($dataBody->products, $userCreated->id, $_paymentOrder);
